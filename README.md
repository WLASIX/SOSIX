...
# SOSIX — автономный браузерный агент (v5.x)

Коротко: SOSIX — экспериментальный автономный агент для управления браузером через Playwright и LLM (NVIDIA Integrate). Он делает итеративный цикл: анализ страницы → решение (JSON) → действие, с упором на семантику элементов, а не на хардкод-селекторы.

Кому подходит: исследователям агентов, инженерам по автоматизации, тем, кто любит экспериментировать с LLM+browser automation. Не использовать для критичных платежей в продакшне.
...
- Может ошибаться и требовать вмешательства/коррекции
- Не гарантируется корректный checkout/оплата на произвольных сайтах.
- CAPTCHAs и 2FA требуют вмешательства пользователя (`wait_for_user_action`).
...
- Для сложных сценариев лучше писать специализированные скрипты, а не полагаться на общий агент.
